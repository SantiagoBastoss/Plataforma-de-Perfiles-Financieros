...
contract Verificacion{
    ...
    function verificar_zkp(uint _cedula, uint _claveReal, uint _claveIngresada) external view returns(bool resultado){

        uint256 docCliente = uint256(_cedula);
        uint256 claveReal = uint256(_claveReal);
        uint256 claveIngresada = uint256(_claveIngresada);

        uint256 clave_secreta = calcular_clave_secreta(docCliente+nitBanco, claveReal, claveBanco);

        resultado = zkp.verifyProof(_pA, _pB, _pC, [clave_secreta, docCliente+nitBanco, claveIngresada, claveBanco]);
        return resultado;
    }

    function calcular_clave_secreta(uint _a, uint _b, uint _c) private pure returns (uint256 clave_secreta) {
